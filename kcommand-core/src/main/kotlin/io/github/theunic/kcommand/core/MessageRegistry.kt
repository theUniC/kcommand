...
class MessageRegistry<M : Any> {
    ...
    fun typeNameAndSerializer(kclass: KClass<out M>): Pair<String, KSerializer<out M>>? = classToEntry[kclass]

    fun serializerFor(typeName: String): KSerializer<out M>? = nameToSerializer[typeName]
}
